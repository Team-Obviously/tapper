 ...
 */
function determineMessageType(similarity, profileMatching) {
    ...
    const careerMatch = profileMatching.some(field => ['company', 'position', 'experience', 'isHiring'].includes(field)) && profileMatching.indexOf('interests') > 2;
...
export async function generateMessage(req, res) {
    try {
        ...
        const message = await claudeService.generatePersonalizedMessage({
            ...
            resumeContent,
        }, messageType, sharedInterests);
        // Generate Telegram URL
        const telegramUrl = generateTelegramUrl(message, toUser.telegramId);
 ...
 */
function generateTelegramUrl(message, telegramId) {
 ...
 */
export async function getUserTelegramId(req, res) {
    try {
