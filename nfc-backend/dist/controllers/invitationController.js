...
export const sendInvitation = async (req, res) => {
    try {
        ...
        const existingInvitation = await db
            .select()
            .from(invitations)
            .where(and(eq(invitations.fromUserId, fromUserId), eq(invitations.toUserId, toUserId), eq(invitations.interest, interest), eq(invitations.status, 'pending')))
            .limit(1);
...
export const getPendingInvitations = async (req, res) => {
    try {
        ...
        const pendingInvitations = await db
            ...
            .where(and(eq(invitations.toUserId, userId), eq(invitations.status, 'pending')))
...
export const getAcceptedConnections = async (req, res) => {
    try {
        ...
        const connections = await db
            ...
            .leftJoin(users, or(and(eq(acceptedConnections.userId1, userId), eq(acceptedConnections.userId2, users.id)), and(eq(acceptedConnections.userId2, userId), eq(acceptedConnections.userId1, users.id))))
            .where(or(eq(acceptedConnections.userId1, userId), eq(acceptedConnections.userId2, userId)))
...
export const getUsersWithSharedInterests = async (req, res) => {
    try {
